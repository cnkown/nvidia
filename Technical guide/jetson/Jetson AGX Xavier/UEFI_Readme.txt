...
Limitations
    ...
    On Jetson Xavier NX, when booting in device tree mode the kernel by default will disable the clocks needed for runtime variable operation, this can be disabled by passing "clk_ignore_unused" as a command line option to the kernel.
    Only SDR25 is supported for SD-cards when booting Linux with ACPI Users may use alternative storage devices, such as USB mass storage, for better performance
    Graphics Output Protocol (GOP) is not supported and so only serial console support is available
...
Version History
    ...
    1.1.1:
        Added support for configuring SPCR/DBG2 and added support for sub-type 5 entries for Operating Systems that support that.
        ...
        Reserve USB firmware region to prevent issues where Operating Systems could corrupt data
...
Extract the nvidia-l4t-jetson-uefi-R32.6-20210809062450.tbz2 package provided over the top of the extracted driver package and navigate to the Linux_for_Tegra subdirectory.

$ tar xpf nvidia-l4t-jetson-uefi-R32.6-20210809062450.tbz2
$ cd Linux_for_Tegra
...
The OS hardware description can be changed without flashing the device as well, see â€œBooting Linux with Device-Tree/ACPIâ€.
...
Where <config> is: - jetson-xavier-uefi for Jetson AGX Xavier - jetson-xavier-nx-uefi-emmc for Jetson Xavier NX (eMMC) - jetson-xavier-nx-uefi-sd for Jetson Xavier NX (SD-card)

Where <dtb> is: - tegra194-p2972-0000.dtb for Jetson AGX Xavier - tegra194-p3509-0000+p3668-0001.dtb for Jetson Xavier NX (eMMC) - tegra194-p3509-0000+p3668-0000.dtb for Jetson Xavier NX (SD-card)
...
Where <config> is: - jetson-xavier-uefi-acpi for Jetson AGX Xavier - jetson-xavier-nx-uefi-acpi-emmc for Jetson Xavier NX (eMMC) - jetson-xavier-nx-uefi-acpi-sd for Jetson Xavier NX (SD-card)
Booting Linux with Device-Tree/ACPI

To change between device tree and ACPI at boot the following steps can be used:

    Press the Escape key when console displays "Press ESCAPE for boot options"
    Select "Device Manager" from the menu.
    Select "O/S Hardware Description Selection" from the menu.
    To select Device Tree, scroll down and select "Device Tree".
    To select ACPI, scroll down and select "ACPI".
    Press the Escape key to go back to previous screen.
    At the prompt, press "Y"
    Press the Escape key once again to go to UEFI menu.
    Select "Reset"
...
PCIe support for ACPI can be enabled with the following steps:
    ...
    Press escape when the console displays "Press ESCAPE for boot options"
    Select "Device Manager" from the menu.
    Select "NVIDIA Resource Configuration" from the menu.
    Select "PCIe Configuration" from the menu.
    Change "Enable PCIe in OS" to Enabled.
    Press the Escape key twice to go back to the previous screen.
    At the prompt, press "Y".
    Press Escape once again to go to the UEFI menu.
    Select "Reset"
